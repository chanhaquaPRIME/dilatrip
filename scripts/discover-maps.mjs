...
const TARGETS = { visit: 50, cafe: 50, eatery: 50, treat: 50, restaurant: 50, stay: 50, nightlife: 50, activity: 50, shopping: 50 };
const BOX = CFG.box; // anything outside the city box is rejected
const BASE_Q = {
  visit: "địa điểm tham quan du lịch",
  cafe: "quán cà phê",
  eatery: "quán ăn sáng quán ăn ngon",
  treat: "kem trà sữa chè bánh ngọt tráng miệng ăn vặt",
  restaurant: "nhà hàng lẩu nướng buffet",
...
const DEFAULTS = {
  ...
  eatery: { hours: "06:30-21:00", visitMin: 60, price: 1, personas: ["solo", "couple", "friends", "family"], desc: "Quán ăn ngon" },
  treat: { hours: "09:00-22:00", visitMin: 45, price: 1, personas: ["friends", "couple", "family"], desc: "Đồ ăn vặt / tráng miệng" },
  restaurant: { hours: "10:00-22:00", visitMin: 90, price: 2, personas: ["couple", "family", "friends"], desc: "Nhà hàng" },
...
// Existing discovered places already in THIS city's output file — preserved on re-run.
function loadKeep() {
  if (!existsSync(EXTRA_OUT)) return [];
  const win = {};
  new Function("window", readFileSync(EXTRA_OUT, "utf8"))(win);
  return win[CFG.varName] || [];
}
...
async function main() {
  ...
  const keep = loadKeep();              // existing discovered places for this city (preserved)
  const extras = [];
  const usedIds = new Set(keep.map((p) => p.id));
  ...
  const merged = [...keep, ...extras]; // preserve existing discovered places, append new
  writeFileSync(EXTRA_OUT,
    ...
    `window.${CFG.varName} = ` + JSON.stringify(merged, null, 2) + ";\n", "utf8");
  console.log(`\nDone. Added ${extras.length} new (${merged.length} total) -> ${EXTRA_OUT}`);
